  ...
  const handleEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (message.trim() !== "") {
      if (e.code == "Enter") {
        const m: Event = {
          payload: {
            from: getChat.data?.data.profile.id,
            message: message,
          },
          type: "send_message",
        };
          ...
          <div className="bg-white px-4 py-4 mb-2 justify-between flex">
            <div className="flex gap-4">
              <div className="relative h-[3rem] w-[3rem] rounded-full overflow-hidden">
                <Image
                  src={getChat.data.data.doctor.image}
                  fill
                  sizes="100%"
                  alt={getChat.data.data.doctor.name}
                />
              </div>
              <div className="flex flex-col justify-center">
                <h3 className="text-[1rem]">
                  Dr. {getChat.data.data.doctor.name}
                </h3>
                <p>
                  <OrderStatus status={getChat.data.data.doctor.status} />
                </p>
              </div>
            </div>
            {getChat.data.data.status !== "ended" &&
              getChat.data.data.status !== "canceled" && (
              ...
              <>
                {chat.map((message, index) => (
                  <div
                    key={index}
                    className={`chat ${
                      message.user_id === getChat.data?.data.profile.id
                        ? "chat-end"
                        : "chat-start"
                    }`}
                  >
