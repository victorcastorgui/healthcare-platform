...
import { DrugData, ProductData, ProductSuper } from '@/types';
...
function EditProductData() {
  ...
  const url = `${apiBaseUrl}/products/${query.edit}/admin`;
  const { data: editData } = useCustomSWR<ProductSuper>(
  ...
  useEffect(() => {
    if (editData) {
      for (const key in editData.data) {
        ...
        setFormData(prevData => {
          return { ...prevData, [keyType]: editData.data[keyType] };
        });
      }
      setPreview(editData.data.image);
      for (const key in editData.data) {
  ...
  const handleSubmitData = async () => {
    ...
    if (isDrug) {
      for (const key in formDataDrug) {
        const keyType = key as keyof DrugData;
        if (
          keyType === 'drug_classification_id' ||
          keyType === 'content' ||
          keyType === 'drug_form_id' ||
          keyType === 'generic_name'
        ) {
          productData.delete(`${keyType}`);
        }
        productData.append(`${keyType}`, formDataDrug[keyType]!.toString());
      }
    }
    const URL = `${apiBaseUrl}/products/${query.edit}`;
    const options = {
      method: 'PUT',
      headers: {
  ...
  return (
    <AdminLayout>
      ...
      <AdminHeader>Edit Product Data</AdminHeader>
      <div className='mt-8 flex flex-col gap-4 max-lg:items-center'>
        ...
        <div className='grid grid-cols-4 gap-4'>
          ...
          <div className='flex col-span-4 gap-4 max-sm:flex-col'>
            {formData && formData.product_category_id !== 0 && (
              <SelectField
              ...
              {formDataDrug && (
                <>
                  <SelectField
                    ...
                    value={formDataDrug.drug_classification_id}
                  />
                  <SelectField
                    onChange={handleChangeDrugs}
                    ...
                    value={formDataDrug.drug_form_id}
                  />
                  <InputField
                    type='text'
                    name='generic_name'
