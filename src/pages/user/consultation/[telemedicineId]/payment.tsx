...
const TelemedicinePayment = () => {
  ...
  const telemedicineDetail = useCustomSWR<TelemedicineDetail>(
    teleId !== undefined ? `${apiBaseUrl}/telemedicines/${teleId}` : null
  );
  const {
  ...
  const handleSubmit = async () => {
    if (paymentProof) {
      ...
      await uploadPayment(`${apiBaseUrl}/telemedicines/${teleId}/payment`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    if (res.message === "payment success") {
      ...
      setData("");
      if (!isLoading) {
        router.push("/user/consultation-history");
      }
      return;
    }
    if (error !== null) {
      toast.error(capitalizeFirstChar(errorMsg as string));
    }
  }, [dataUploadPayment, error, router, isLoading, errorMsg, setData]);
            ...
            <div className="flex flex-col col-span-2 gap-4">
              ...
              <BaseCard>
                ...
                <div className="max-w-44 mt-6">
                  <Button variants="secondary" onClick={handleSubmit}>
                    Upload
                  </Button>
                </div>
              </BaseCard>
